...
io.sockets.on("connection", function (socket) {
    ...
    socket.on("chatMessage", data => {
        if (typeof data[1] === 'string') {
            if(data[5] == 'image'){
                ...
                console.log('CHAT: file => user[' + data[0]["name"] + "] session[" + data[2] + "] file[" + data[1] + "] id[" + data[4] + "]");
                io.emit("chatMessage_file", {
                    user: data[0],
                    message: data[1],
                    session: data[2],
                    unread: data[3],
    ...
    socket.on("disconnect", (reason) => {
        if (users[socket.id] && reason != 'ping timeout') { // && reason != 'ping timeout' && reason != 'transport close' && reason != 'transport error'
