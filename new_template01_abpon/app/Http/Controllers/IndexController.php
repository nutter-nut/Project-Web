...
// dd($data);
    ...
    public function cartIndex(Request $request)
    {
        ...
        foreach($cart->items as $key => $item){
            ...
            $data['data'][$key]['arr_qty_mian'] = $item['quantity'] . "-" . $item['data']['uomCode'];
        }
...
// dd($data);
    ...
    public function findProducts($id)
    {
        ...
        foreach($get_products as $key => $item){
            $new_prodoucts[$key]['posone'] = $item;
            $new_prodoucts[$key]['web'] = app('App\Http\Controllers\ProductsController')->getProduct($item[0]->prodCode, $item[0]->uomCode);
        }

        return $new_prodoucts;
    }
