  ...
  <div class="chatbox__body" style="padding: 0%">
    <div class="container">
      <div class="row">
        ...
        <!-- margin-right: 15px; -->
        <div class="col-8" style="padding-left: 0%; padding-right: 0px;">
          <message-component
  ...
  methods: {
    ...
    sendFriendId(friend) {
      if (friend) {
        ...
        // ----------------------------------
        axios.post("/recount_unread", { session: friend.session }).then((r) => {
          if(r.data){
            this.loadReaded();
          }
        });
    ...
    lastMessage(socket) {
      ...
      this.$root.socket.on(socket, function (data) {
        if (vm.user.id == data.user.id) {
          (typeof data.message === "string") ? (vm.friend_list[vm.index].unread_message = "คุณ: รูปภาพ") : (vm.friend_list[vm.index].unread_message = "คุณ: " + data.message.message);
          vm.friend_list.push();
        } else {
          var i = 0;
          vm.friend_list.forEach((element) => {
            if (element.id == data.user.id && element.session == data.session) {
              (typeof data.message === "string") ? (vm.friend_list[i].unread_message = "รูปภาพ") : (vm.friend_list[i].unread_message = data.message.message);
              vm.friend_list.push();
              if (vm.index != i) {
                $(document).ready(function () {
