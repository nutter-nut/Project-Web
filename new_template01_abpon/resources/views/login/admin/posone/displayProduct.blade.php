                            ...
                            <table id="table_id" class="table table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col">{{ __('login.page.products.display.product_stock') }}</th>
                                        <th scope="col">{{ __('login.page.products.display.product_status') }}</th>
                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <table class="table table-sm">
                                                    @foreach($items['stock'] as $item_stock)
                                                    <tr>
                                                        <td class="@if($item_stock->endQty < 0) text-danger @endif">{{ number_format($item_stock->endQty) }} ({{ $item_stock->uomName }})</td>
                                                        <td><a href="{{ route('stockEdit', [ 'whCode' => $items['treasury'], 'stockProdCode' => $item->prodCode, 'uom' => $item_stock->prodUnit1UOMCode ]) }}" class="btn btn-sm btn-outline-success"> <i class="fa fa-plus"></i></a></td>
                                                    </tr>
                                                    @endforeach 
                                                </table>
                                            </td>
                                            <td>
                                                @if($items['menugroup'] != null)
                                                    <span class="label label-success">กำลังขาย</span>
                                                @else
                                                    <span class="label label-warning">รอวางขาย</span>
                                                @endif
