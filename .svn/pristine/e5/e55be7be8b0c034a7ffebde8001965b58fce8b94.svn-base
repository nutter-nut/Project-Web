    ...
    methods: {
        videoTime() {
            ...
            this.$root.socket.on("videoTime", function(data) {
                if (vm.session == data["session"] && data["status"] == "start" && vm.user.id == data["user_id"]) {
                    axios
                        .post("/video_time", {
                            data: {
                    ...
                    data["session"] = "";
                } else if (vm.session == data["session"] && data["status"] != "start") {
                    axios
                        .post("/video_time_end", {
                            data: {
                                session: data["session"],
        ...
        reading() {
            ...
            this.$root.socket.on("toChat", function(data) {
                if (vm.session == data.session["id"] && vm.user.id != data.user_id.id) {
                    axios.post("/reading", {
                        data: {
        ...
        chatMessagesFile() {
            let vm = this;
            this.$root.socket.on("chatMessage_file", function(data) {
                if (vm.session == data["session"]) {
                    vm.messages.push({
                        id: data["user"]["id"],
                        name: data["user"]["name"],
